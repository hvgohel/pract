...
public class DataSourceProvider {
    ...
    public DataSource getDataSource() throws Exception {
        ...
        if (StringUtils.isBlank(url) || StringUtils.isBlank(username) || StringUtils.isBlank(password)) {
            throw new RuntimeException(String.format("Either of these properties not found : %s, %s or %s",
                    PROPERTY_URL, PROPERTY_USERNAME, PROPERTY_PASSWORD));
        }

        // url will contain '&amp;' as it comes from configuration but we need to interpret it as '&' only so
        // replacing '&amp;' with '&'. Here it is not the proper way to do this but we don't found any effective way to
